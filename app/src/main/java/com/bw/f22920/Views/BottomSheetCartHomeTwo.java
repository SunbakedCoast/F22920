...
import com.bw.f22920.Utilities.CartAdapter.CartFragRViewAdapterHomeTwo;
...
public class BottomSheetCartHomeTwo extends BottomSheetDialog {
    private List<CartArrayModelHometwo> itemlist = new ArrayList<>();
    View view;
    TextView sendcartbtn;


    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.bottom_sheet_cart_hometwo, container, false);
        sendcartbtn = view.findViewById(R.id.bottom_sheet_cart_sendbtn);

        final NavController navigator = Navigation.findNavController(getActivity(), R.id.fragment_host);

        sendcartbtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                navigator.navigate(R.id.action_bottomSheetCartHomeTwo_to_fragment_Home);
            }
        });

        DatabaseReference db = FirebaseDatabase.getInstance().getReference("itemdata");
        db.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()){
                    sendcartbtn.setVisibility(View.VISIBLE);
                } else {
                    sendcartbtn.setVisibility(View.GONE);
                }
            }
        ...
        data.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.hasChild("potato")){
                    ...
                    CartArrayModelHometwo item = new CartArrayModelHometwo(dataname, dataquantity,dataprice );
                    itemlist.add(item);
                }
    ...
    public void execRecyclerView(){
        RecyclerView recyclerView = view.findViewById(R.id.bottom_sheet_cart_rv_hometwo);
        CartFragRViewAdapterHomeTwo adapter = new CartFragRViewAdapterHomeTwo(getActivity(), itemlist);
